...
/**
 * Collect every image block: `{ messageIndex, blockIndex, id }`.
...
export function collectImageBlocks(messages) {
  ...
  for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
    ...
    for (let blockIndex = 0; blockIndex < content.length; blockIndex += 1) {
      ...
      if (block && block.type === 'image' && block.attachment && typeof block.attachment.id === 'string') {
        blocks.push({ messageIndex, blockIndex, id: block.attachment.id })
      }
    }
  }
  return blocks
...
export async function replaceImages(ctx, settings, messages, signal, cache, stats, log) {
  ...
  await Promise.all(pending.map(async (id) => {
    ...
    try {
      const raw = await callVisionModel(ctx, vision, id, signal)
  ...
  return messages.map((message, messageIndex) => {
    ...
    if (!Array.isArray(content) || !content.some((block) => block && block.type === 'image')) return message
    const replaced = content.map((block, blockIndex) => {
      if (!block || block.type !== 'image' || !block.attachment) return block
      const hit = blocks.find((b) => b.messageIndex === messageIndex && b.blockIndex === blockIndex)
      const id = hit !== undefined ? hit.id : block.attachment.id
...
async function callVisionModel(ctx, vision, attachmentId, signal) {
  const config = {
    ...
    messages: [
      {
        ...
        content: [
          ...
          { type: 'image', attachment: { id: attachmentId } },
        ],
