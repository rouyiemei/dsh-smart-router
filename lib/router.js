...
export class SmartRouterAdapter extends LlmAdapter {
  ...
  async *stream(options) {
    const settings = this.config()
    // Diagnostic: log message count and whether any contain images
    const imgCount = (options.messages ?? []).filter(
      (m) => Array.isArray(m?.content) && contentHasImage(m.content),
    ).length
    this.log(`smart-router: stream called — ${options.messages?.length ?? 0} messages, ${imgCount} with images, visionMode=${settings.visionMode}`)
  ...
  async prepareMessages(options, settings) {
    ...
    this.log(`smart-router: prepareMessages — hasImage=${hasImage}, visionMode=${settings.visionMode}`)
