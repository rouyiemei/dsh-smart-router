...
export class SmartRouterAdapter extends LlmAdapter {
  ...
  resolveChain(options) {
    ...
    const hasImage = last !== undefined && contentHasImage(last.content)
    ...
    if (hasImage) {
      ...
      const vision = normalizeRoute(tierRoute(settings, 'vision'))
      if (routeConfigured(vision)) chain.push({ ...vision, reason: 'vision tier' })
      for (const fb of Array.isArray(settings.visionFallbacks) ? settings.visionFallbacks : []) {
        const route = normalizeRoute(fb)
        if (routeConfigured(route) && !chain.some((c) => c.provider === route.provider && c.model === route.model)) {
          chain.push({ ...route, reason: 'vision fallback' })
        }
      }
      if (routeConfigured(defaultRoute) && !chain.some((c) => c.provider === defaultRoute.provider && c.model === defaultRoute.model)) {
        chain.push({ ...defaultRoute, reason: 'default model' })
      }
    ...
    if (text !== '') {
      if (String(settings.classifier) === 'llm') {
        const llm = this.classifyWithLlm(text, options.signal)
        if (llm !== undefined) {
          level = llm.level
    ...
    if (routeConfigured(requested)) chain.push({ ...requested, reason: `${level} tier` })
    for (const tier of TIER_ORDER) {
      if (tier === level) continue
      const route = normalizeRoute(tierRoute(settings, tier))
      if (routeConfigured(route) && !chain.some((c) => c.provider === route.provider && c.model === route.model)) {
        chain.push({ ...route, reason: `${tier} tier (fallback)` })
      }
    }
    if (routeConfigured(defaultRoute) && !chain.some((c) => c.provider === defaultRoute.provider && c.model === defaultRoute.model)) {
      chain.push({ ...defaultRoute, reason: 'default model' })
  ...
  async *stream(options) {
    const settings = this.config()
    const resolved = this.resolveChain(options)
  ...
  /** Delegate one request to a concrete provider/model via the prepared-call contract. */
  async *delegate(options, target) {
    const { reasoningEffort: _inherited, ...rest } = options
    const config = {
    ...
    yield* prepared.stream(forwarded)
  }
}
